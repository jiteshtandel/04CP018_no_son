jQuery(document).ready(function(e) {
    var nativecountry=jQuery("#field_2315").val();
    var currentcountry=jQuery("#field_2317").val();
    ...
    jQuery("#profile-edit-form").validate({
            ...
            rules:{
                    ...
                    field_2337:{
                            required: {
                                depends: function(element) {
                                    return ((jQuery("#field_2359").val()!="")? true : false);
                                }
                            }
                    ...
                    field_2344:{
                            required: {
                                depends: function(element) {
                                    return ((jQuery("#field_2360").val()!="")? true : false);
                                }
                            }
                    },//Language Level 3
                    field_2323:{
            ...
            messages:{
                    ...
                    field_2315:{
                            ...
                            checkcountry:"Please enter proper country"
                    },//native Country
                    field_2317:{
                    ...
                    field_2358:{
                            ...
                            checklanguage:"Please enter proper language"
                    },//Language Learning 1
                    field_2325:{
    ...
    setCountries();
    bindNativeStateAutoComplete(nativecountry);
    bindCurrentStateAutoComplete(currentcountry);
    bindLanguagesAutoComplete();
    
});

jQuery.validator.addMethod('checkcountry', function (value, element) {
    if (city_states[value])
        return true;
    else
        return false;     
});

jQuery.validator.addMethod('checklanguage', function (value, element) {
    if(value!="" && value!=null){
...
jQuery.validator.addMethod("checkstate", function(value, element, params) {
    var country=jQuery(params).val();
    if(value!="" && value!=null && country!="" && country!=null){
        return checkstateExistInCountry(value,country);
    }
    else{
        return true;
    }    
}, jQuery.format("Please enter the correct state for respective country"));
...
function setCountries(){
    ...
    bindCountriesAutoComplete(countriesArr);
}

function bindCountriesAutoComplete(countriesArr){
     // Initialize autocomplete with local lookup:
    jQuery('#field_2315,#field_2319').autocomplete({
        lookup: countriesArr,
        minChars: 0,
        onSelect: function (suggestion) {
            var fieldid=jQuery(this).attr("id");
            if(fieldid=="field_2315"){
                //jQuery("#field_2317").val("");
                bindNativeStateAutoComplete(suggestion.value);
            }
            else{
                //jQuery("#field_2321").val("");
                bindCurrentStateAutoComplete(suggestion.value);
            }
        },
...
function bindLanguagesAutoComplete(){
    // Initialize autocomplete with local lookup:
    jQuery('#field_2358,#field_2359,#field_2360,#field_2323,#field_2334,#field_2342').autocomplete({
        lookup: languages,
        minChars: 0,
        showNoSuggestionNotice: true,
        noSuggestionNotice: 'Sorry, no matching results'
...
function bindNativeStateAutoComplete(country){
    if(city_states[country]){
        var stateArr=city_states[country].split('|');
        stateArr.sort();
        jQuery("#field_2317").autocomplete({
            lookup: stateArr,
            minChars: 0,
            onSelect: function (suggestion) {
            },
...
function bindCurrentStateAutoComplete(country){
    if(city_states[country]){
        var stateArr=city_states[country].split('|');
        stateArr.sort();
        jQuery("#field_2321").autocomplete({
            lookup: stateArr,
            minChars: 0,
            onSelect: function (suggestion) {
            },
...
function checkstateExistInCountry(state,country){
    if (city_states[country]){
        var stateArr = city_states[country].split('|');
